...
from deliver_ai.deliverai_utils import Person, Ticket, Bot
from deliver_ai.server import get_server, terminate_server
...
def create_app(test_config=None):
    ...
    app.config.from_mapping(
        SECRET_KEY='dev',
        # DATABASE=os.path.join(app.instance_path, 'deliver_ai.sqlite'),
    )
    ...
    # register blueprints
    from deliver_ai import send
    app.register_blueprint(send.bp)

    with app.app_context():
        # users
        # people = Person.from_file(os.path.join(app.instance_path, '..', 'people.txt'))
        # people_map = {person.username: person for person in people}
        # user = people_map.pop("ash.ketchum", None)
        #
        # # robots
        # bots = Bot.from_file(os.path.join(app.instance_path, '..', "bots.txt"))
        #
        # tickets = []
        #
